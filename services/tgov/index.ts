...
import { MeetingRecordModel } from "./db/models/db";
...
type ListMeetingsParams = {
  hasUnsavedAgenda?: Query<boolean>;
  committeeId?: Query<string>;
  beforeDate?: Query<Date>;
  afterDate?: Query<Date>;
  cursor?: Query<string>;
  prev?: Query<number>;
  next?: Query<number>;
  sort?: Query<"name" | "startedAt" | "committee">;
...
export const listMeetings = api(
  ...
  ): Promise<{ meetings: MeetingRecordModel[]; total: number }> => {
    try {
      const where: Prisma.MeetingRecordWhereInput = {};
      const orderBy: Prisma.MeetingRecordOrderByWithRelationInput = {};
      ...
      if (params.sort === "committee") {
        orderBy.committee = {
          name: (params.sortDirection || "asc") as "asc" | "desc",
        };
      } else if (params.sort) {
        orderBy[params.sort as "name" | "startedAt"] = (params.sortDirection ||
          "desc") as "asc" | "desc";
      }
