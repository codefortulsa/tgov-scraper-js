...
import { tgov, media, transcription } from '~encore/clients';
...
describe('End-to-end transcription flow', () => {
  ...
  beforeAll(async () => {
    tempDir = path.join(os.tmpdir(), `tulsa-transcribe-test-${uuidv4()}`);
  ...
  test('Scrape TGov website', async () => {
    // Trigger a scrape of the TGov website
    const result = await tgov.scrape();
    expect(result.success).toBe(true);
  }, TEST_TIMEOUT);

  test('Get meeting list and extract video URL', async () => {
    ...
    if (meeting.videoViewUrl) {
      const extractResult = await tgov.extractVideoUrl({ viewerUrl: meeting.videoViewUrl });
  ...
  test('Queue video for download and processing', async () => {
  ...
  test('Process the video batch', async () => {
    // Process the queued batch
    const processResult = await media.processNextBatch();
    ...
    while (!batchComplete && attempts < maxAttempts) {
      ...
      if (statusResult.status === 'completed' || 
        ...
        throw new Error(`Batch processing failed: ${JSON.stringify(statusResult)}`);
      } else {
  ...
  test('Submit audio for transcription', async () => {
  ...
  test('Wait for transcription to complete', async () => {
    ...
    while (!transcriptionComplete && attempts < maxAttempts) {
      ...
      if (jobStatus.status === 'completed') {
        ...
        expect(transcriptionDetails.segments.length).toBeGreaterThan(0);
  ...
  test('Verify database records for meeting', async () => {
